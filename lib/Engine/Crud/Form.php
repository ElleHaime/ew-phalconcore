	...
	InjectionAwareInterface
{
	...
	protected function _setupForm()
	{
		...
		foreach ($this->_fields as $key => $field) {
			/* @var \Engine\Crud\Form\Field $field */
	...
	public function addField($field, $key = null, array $options = [])
	{
		if (is_string($field)) {
			if (null === $key) {
				throw new Exception('Fields specified by string must have an accompanying name');
			}
	...
	public function offsetSet($key, $value)
	{
		if (is_null($key)) {
			...
			throw new Exception('Field with key \''.$key.'\' not exists');
		}
	...
	{
		throw new Exception("Can not unset field in the form");
	}
	...
	public function __get($key)
	{
		if (is_null($key)) {
			...
			throw new Exception('Field key can not be empty');
		} elseif (!isset($this->_fields[$key])) {
			throw new Exception('Field \''.$key.'\' is not in the form');
		}
