...
class DateTime {
    ...
    public static function timeAgoInWords($dateTime, $options = array()) {
        ...
        $dependencyInjectorff = $futureTime - $pastTime;

        if (!$dependencyInjectorff) {
            return sprintf('just now', 'just now');
        }

        if ($dependencyInjectorff > abs($now - self::fromString($end))) {
            return sprintf($absoluteString, date($format, $inSeconds));
        }

// If more than a week, then take into account the length of months
        if ($dependencyInjectorff >= 604800) {
            ...
            if (!$months && $years >= 1 && $dependencyInjectorff < ($years * 31536000)) {
                $months = 11;
                $years--;
            }
            ...
            $days = floor($dependencyInjectorff / 86400);

            $dependencyInjectorff = $dependencyInjectorff - ($days * 86400);

            $hours = floor($dependencyInjectorff / 3600);
            $dependencyInjectorff = $dependencyInjectorff - ($hours * 3600);

            $minutes = floor($dependencyInjectorff / 60);
            $dependencyInjectorff = $dependencyInjectorff - ($minutes * 60);
            $seconds = $dependencyInjectorff;
        }
