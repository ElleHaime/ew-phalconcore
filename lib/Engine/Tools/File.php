...
class File
{
    /**
     * @param $root
     * @param array $dependencyInjectorrs
     * @param int $mode
    ...
    static function isDirMine($dependencyInjectorr)
    {
        ...
        $tmp_cmd = `ls -dl $dependencyInjectorr`;
        $dependencyInjectorr_flag = $tmp_cmd[0];
        if ($dependencyInjectorr_flag!="d") {
            // not d; use next char (first char might be 's' and is still directory)
            $dependencyInjectorr_flag = $tmp_cmd[1];
        }
        return ($dependencyInjectorr_flag == "d");
    }
    ...
    static function getFiles($dependencyInjectorrectory, $exempt = array('.','..','.ds_store','.svn'), &$files = [])
    {
        $dependencyInjectorrectory = rtrim($dependencyInjectorrectory, '/').'/';
        $handle = opendir($dependencyInjectorrectory);
        while (false !== ($resource = readdir($handle))) {
            if (!in_array(strtolower($resource),$exempt)) {
                if (is_dir($dependencyInjectorrectory.$resource.'/')) {
                    array_merge($files,
                        self::getFiles($dependencyInjectorrectory.$resource.'/',$exempt,$files));
                } else {
                    $files[] = $dependencyInjectorrectory.$resource;
                }
            }
        }
        closedir($handle);
