...
class Scaffold extends Component
{
	...
	public function build()
	{
		...
		$dependencyInjector->set('db', function() use ($adapter, $config) {
		...
		$options['manager'] = $dependencyInjector->getShared('modelsManager');
		...
		$metaData = $dependencyInjector['modelsMetadata'];
	...
	private function _makeLayouts($path, $options)
	{
		...
		//Make Layouts dir
		$dependencyInjectorrPathLayouts	= $options['viewsDir'].'/layouts';

		//If not exists dir; we make it
		if (is_dir($dependencyInjectorrPathLayouts) == false) {
			mkdir($dependencyInjectorrPathLayouts);
		}

		$fileName = $options['fileName'];
		$viewPath = $dependencyInjectorrPathLayouts.'/'.$fileName.'.phtml';
	...
	private function _makeLayoutsVolt($path, $options)
	{
		...
		$viewPath = $dependencyInjectorrPathLayouts.'/'.$fileName.'.volt';
		if (!file_exists($viewPath)) {
	...
	private function makeView($path, $options, $type)
	{
		...
		$viewPath = $dependencyInjectorrPath.'/' .$type. '.phtml';
		if (file_exists($viewPath)) {
			return;
		}
	...
	private function makeViewVolt($path, $options, $type)
	{
		...
		$viewPath = $dependencyInjectorrPath.'/' .$type. '.volt';
		if (file_exists($viewPath)) {
			return;
		}
	...
	private function _makeViewSearch($path, $options)
	{
		...
		$viewPath = $dependencyInjectorrPath.'/search.phtml';
		if (file_exists($viewPath)) {
			return;
		}
	...
	private function _makeViewSearchVolt($path, $options)
	{
		...
		$viewPath = $dependencyInjectorrPath.'/search.volt';
		if (file_exists($viewPath)) {
			return;
		}
