...
class Number {
    ...
    public static function toReadableSize($size)
    {
        switch (true) {
            ...
            case round($size / 1024) < 1024:
                return sprintf('%s KB', self::precision($size / 1024, 0));
            case round($size / 1024 / 1024, 2) < 1024:
                return sprintf('%s MB', self::precision($size / 1024 / 1024, 2));
            case round($size / 1024 / 1024 / 1024, 2) < 1024:
                return sprintf('%s GB', self::precision($size / 1024 / 1024 / 1024, 2));
            default:
                return sprintf('%s TB', self::precision($size / 1024 / 1024 / 1024 / 1024, 2));
        }
    }
    ...
    /**
     * @param integer|string $value floating point number
     * @param array|bool $options If integer then places, if string then before, if (,.-) then use it
     * or array with places and before keys
     * @return string formatted number
     */
    ...
    /**
     * Add a currency format. Makes reusing
