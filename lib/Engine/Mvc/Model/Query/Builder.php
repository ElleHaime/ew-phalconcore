...
class Builder extends PhBuilder
{
     ...
     * @return string
     */
    public function getCorrelationName($col, $colModel = null, $reverse = false)
    {
        if ($col === '*') {
        ...
        if ($reverse) {
            $correlationName = $this->_findJoinCorrelationName($col, $colModel, $reverse);
            if (!$correlationName) {
                $correlationName = $this->_findFromCorrelationName($col, $colModel);
            }
        } else {
            $correlationName = $this->_findFromCorrelationName($col, $colModel);
            if (!$correlationName) {
                $correlationName = $this->_findJoinCorrelationName($col, $colModel);
            }
        }
     ...
     * @return string
     */
    private function _findFromCorrelationName($col, $colModel = null)
    {
        $correlationName = false;
        ...
        foreach ($correlationNameKeys as $key => $modelName) {
            ...
            if (in_array($col, $cols)) {
                ...
                $correlationName = (is_numeric($key) ? $modelName : $key);
            }
        }
     ...
     * @return string
     */
    private function _findJoinCorrelationName($col, $colModel = null, $reverse = false)
    {
        $correlationNameKeys = ($reverse === true) ? array_reverse($this->_joins) : $this->_joins;
