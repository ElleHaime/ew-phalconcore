...
class AnnotationsMetaDataInitializer
{
    ...
    {
        $reflection = $dependencyInjector['annotations']->get($model);
     ...
     *
     * @param DiInterface $dependencyInjector Dependency Injection.
     *
     * @return array
     */
    public function getAllModelsMetadata(DiInterface $dependencyInjector)
    {
        $models = [];
        foreach ($dependencyInjector->get('modules') as $module => $enabled) {
            if (!$enabled) {
                continue;
            }
            $modelsDirectory = $dependencyInjector->get('config')->application->modulesDir . ucfirst($module).'/Model';
            foreach (glob($modelsDirectory.'/*.php') as $modelPath) {
                $modelInfo = [];
                $modelClass = '\\' . ucfirst($module).'\Model\\' . basename(str_replace('.php', '', $modelPath));
                $reflector = $dependencyInjector->get('annotations')->get($modelClass);
