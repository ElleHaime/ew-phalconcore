...
class Database extends Adapter implements AdapterInterface
{
	 ...
	 */
	public function __construct($options, \Phalcon\DiInterface $dependencyInjector)
	{
	...
	protected function _allowOrDeny($roleName, $resourceName, $access, $action, $func = null)
	{
		...
		if (is_array($access)) {
			foreach ($access as $accessName) {
				$this->_insertOrUpdateAccess($roleName, $resourceName, $accessName, $action, $func);
			}
		} else {
			$this->_insertOrUpdateAccess($roleName, $resourceName, $access, $action, $func);
		}
